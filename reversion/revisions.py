...
def _copy_db_versions(db_versions):
    return {db: versions.copy() for db, versions in db_versions.items()}
...
def _pop_frame():
    ...
    if is_active():
        current_frame = _current_frame()
        db_versions = {
            db: prev_frame.db_versions[db] for db in current_frame.db_versions.keys()
        }
        _update_frame(
...
def _add_to_revision(obj, using, model_db, explicit):
    ...
    if version_options.ignore_duplicates and explicit:
        previous_version = (
            Version.objects.using(using).get_for_object(obj, model_db=model_db).first()
        )
        if (
            previous_version
            and previous_version._local_field_dict == version._local_field_dict
        ):
            return
    # Store the version.
...
def _save_revision(
    versions, user=None, comment="", meta=(), date_created=None, using=None
):
