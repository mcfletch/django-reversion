...
def _safe_revert(versions):
    ...
    if len(unreverted_versions) == len(versions):
        raise RevertError(
            ugettext("Could not save %(object_repr)s version - missing dependency.")
            % {
                ...
                if delete:
                    ...
                    for version in versions:
                        ...
                        try:
                            ...
                            old_revision.add(
                                model._default_manager.using(version.db).get(
                    ...
                    current_revision = chain.from_iterable(
                        _follow_relations_recursive(obj) for obj in old_revision
                    )
                    # Delete objects that are no longer in the current revision.
                    collector = Collector(using=version_db)
                    new_objs = [
                        item for item in current_revision if item not in old_revision
                    ]
...
class VersionQuerySet(models.QuerySet):
    ...
    def get_deleted(self, model, model_db=None):
        ...
        if self.db == model_db and connection.vendor in (
            "sqlite",
            "postgresql",
            "oracle",
        ):
...
class Version(models.Model):
    ...
    def _object_version(self):
        ...
        try:
            return list(
                serializers.deserialize(self.format, data, ignorenonexistent=True)
            )[0]
    ...
    def field_dict(self):
        ...
        for (
            parent_model,
            field,
        ) in self._model._meta.concrete_model._meta.parents.items():
...
def _safe_subquery(
    ...
    if left_query.db != right_subquery.db or not (
        left_field.get_internal_type() != right_field.get_internal_type()
        and connections[left_query.db].vendor in ("sqlite", "postgresql")
    ):
        ...
        exist_annotation_name = "{}_annotation_str".format(
            right_subquery.model._meta.db_table
        )
        right_subquery = right_subquery.filter(
            **{right_field_name: models.OuterRef(left_field_name)}
        )
        left_query = left_query.annotate(
            **{exist_annotation_name: models.Exists(right_subquery)}
        )
